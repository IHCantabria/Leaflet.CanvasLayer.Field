...
var Field = function () {
    ...
    _createClass(Field, [{
        ...
        value: function _longitudeAtX(i) {
            ...
            var lon = this.xllCorner + halfPixel + i * this.cellSize;
            if (this.longitudeNeedsToBeWrapped) {
...
L.CanvasLayer.ScalarField = L.CanvasLayer.Field.extend({
    ...
    _drawArrows: function _drawArrows() {
        ...
        var stride = Math.max(1, Math.floor(1.2 * this.options.arrowSize / this._field.cellSize));

        var cells = this._field.getCells();
        var ctx = this._getDrawingContext();
    ...
    _drawArrow: function _drawArrow(cell, ctx) {
        var size = this.options.arrowSize;
