describe("VectorField", function () {
    ...
    describe("Positions", function () {
        ...
        it("can return vector data for (i, j) indexes in the grid", function () {
            // top-left
            expect(vf._vector(0, 0)).toEqual([0.011275325901806355, -0.003540651174262166]);

            // botton-right
            expect(vf._vector(9, 9)).toEqual([0.14851005375385284, -0.015279672108590603]);
        });
    ...
    describe("Lon-Lat", function () {
        ...
        it("returns [u, v] values for a Lon-Lat ", function () {
            ...
            var pNearLL = vf.valuesAt(-3.76921724303, 43.4605948227);
            expect(pNearLL[0]).toBeCloseTo(0.00586759205907583, 4);
            expect(pNearLL[1]).toBeCloseTo(-0.00329965655691922, 4);

            // near the center of the UL cell [down & right]
            var pNearUL = vf.valuesAt(-3.76921740247, 43.4651398993);
            expect(pNearUL[0]).toBeCloseTo(0.01127532590180643, 4);
            expect(pNearUL[1]).toBeCloseTo(-0.00354065117426217, 4);

            // near the center of the UR cell [down & left]
            var pNearUR = vf.valuesAt(-3.76467191838, 43.4651400146);
            expect(pNearUR[0]).toBeCloseTo(0.215018898248672, 4);
            expect(pNearUR[1]).toBeCloseTo(-0.00158081843983382, 4);

            // near the center of the LR cell [up & left]
            var pNearLR = vf.valuesAt(-3.76467191746, 43.4605944395);
            expect(pNearLR[0]).toBeCloseTo(0.148510053753853, 4);
            expect(pNearLR[1]).toBeCloseTo(-0.0152796721085906, 4);
        });

        it("doesn't return values for a Lon-Lat outside of the grid", function () {
            let lon = -3.710591474014617;
            let lat = 43.47082043045964;
            expect(vf.hasValuesAt(lon, lat)).toBe(false);
        });
    });
